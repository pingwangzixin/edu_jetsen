...
//	$urlRouterProvider.otherwise('/wrap/index');
	...
	$stateProvider
		...
		.state('wrap.publicEvaluation', { //共用 优秀学生，班主任、任课教师编辑及查看，学生家长查看。评价评语页面
			...
			params : {prevPage : '',state: '',stuId : '',bestStuId : ''},
			resolve: {
				deps: ["$ocLazyLoad", function($ocLazyLoad) {
...
app.directive('topList', function($timeout) {
	return {
		...
		template: '<div class="zy_top_list_enter" ui-sref="wrap.topStudentList({prevPage:page,bestStuId:best})"><img src="./img/door_top_list.png" /><span>2017-2018</span></div>',
		link: function(scope, element, attrs) {
