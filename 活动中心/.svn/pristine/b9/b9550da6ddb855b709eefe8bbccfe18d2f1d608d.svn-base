	...
	$http.get(requireIp+"activity/activitycenter/findActivityCenter?activityId="+$stateParams.activityId).success(function(data) {
	    ...
	    if(data.code==200){
			...
			if(data.data.activityCenter.assignMastersummaryFile !="" && data.data.activityCenter.assignMastersummaryFile != null){
				$.each(JSON.parse(data.data.activityCenter.assignMastersummaryFile), function(i,o) {
				var sharname = {};
				  	var  amfile =o ;
				...
				sharname.edit_file= relName;
				sharname.filedownload=requireIp+"/teacher/upload/downloadByPath.do?fileName="+filen+"&oName="+oName;
				fileArry.push(sharname);
				if(relName !=""){
		    		$scope.deIsShowImg=true;//已上传文件显示
		    	}
				
				})
		  $scope.sharList = fileArry;
			}
		}
	});
