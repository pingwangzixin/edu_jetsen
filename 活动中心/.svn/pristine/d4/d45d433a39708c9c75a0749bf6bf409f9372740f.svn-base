app.controller('sharedLibraryStatisticsDetailCtrl',['$scope','$state','$timeout','$http','$location','$stateParams','$rootScope',function($scope,$state,$timeout,$http,$location,$stateParams,$rootScope) {
...
$scope.ifLike = {
	...
	$http.get(requireIp+"activity/activitycenter/findActivityCenter?activityId="+$stateParams.activityId+"&userId="+teaId).success(function(data) {
	    ...
	    if(data.code==200){
	    	if(!$.isEmptyObject(data.data.activityCenter.activityFile) &&data.data.activityCenter.activityFile !="[]") {
	...
	$scope.checkText = function () {
        if ($scope.commentContext.length > 255) {
	 		$scope.myshow=true;
	 		$scope.tctips="活动评论仅可输入255字符"
	 		$timeout(function(){
	 			$scope.myshow=false;
	 		},1500)
	 		$scope.commentContext = $scope.commentContext.substr(0, 255);
        }
    };
	//提交评论
