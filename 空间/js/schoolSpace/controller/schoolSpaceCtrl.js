app.controller('schoolSpaceCtrl',['$scope','$state','$timeout','$http','$location','$interval',function($scope,$state,$timeout,$http,$location,$interval) {
		// 获取学校Id
   ...
   $http.get(spaceEaIp+"/ea/api/uc/user/"+sessionStorage.getItem("userId"))
	...
	//导航
	$scope.spaceNav = [
		{name : '首页',href : 'wrap.schoolSpace.schoolSpaceIndex'},
		{name : '校训',href : 'wrap.schoolSpace.schoolMotto'},
		{name : '学校介绍',href : 'wrap.schoolSpace.schoolIntro'},
		{name : '学校资讯',href : 'wrap.schoolSpace.schoolNews'},
		{name : '学校风采',href : 'wrap.schoolSpace.schoolPhoto'},
		{name : '空间展示',href : 'wrap.schoolSpace.schoolSpaceShow'}
	];
