app.controller('classNoticeEditCtrl', ['$scope', '$state', '$timeout', '$http', '$location', '$interval', function ($scope, $state, $timeout, $http, $location, $interval) {
	//定义变量
	$scope.constants = {
		enclosureId: [],
		newEnclosureId: [],
	...
	$scope.publish = function (ueditor) {
		if ($scope.state == 0) {
			if(!$scope.ueditor || !$scope.title){
				tipBoxShow(false, "公告名称及内容不能为空")
				return
			}
			var params = {
			...
			$http.post(spaceJeucIp + "/jeuc/api/ea/eaSpaceIntroduce/publishIntroduce?", params).success(function (data) {
				console.log(data);
				if (data.ret == 200) {
