app.controller('schoolNewsListCtrl',['$rootScope','$scope','$state','$timeout','$http','$location','$interval',function($rootScope,$scope,$state,$timeout,$http,$location,$interval) {
	//上传背景隐藏
...
//	//根据登录的用户查询用户信息
		 $http.get(spaceJeucIp + '/jeuc/api/uc/ucUser/'+sessionStorage.getItem("userId")+'/'+sessionStorage.getItem("userType")).success(function(data){
		 	if (data.ret==200) {
		 		var data = data.data;
	...
	.success(function (data) {
		...
		$scope.classNoticeListDel = function(i,id){
			...
			$scope.sureDelete = function (){
				...
				$http.get(spaceJeucIp+"/jeuc/api/ea/eaSpaceIntroduce/deleteEaSpaceIntroduce?id="+id+"&type="+$scope.variable.type)
				.success(function (data) {
					if(data.ret==200){
