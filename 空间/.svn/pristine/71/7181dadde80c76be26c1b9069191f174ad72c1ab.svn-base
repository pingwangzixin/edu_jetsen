app.controller('schoolPhotoListCtrl',['$rootScope','$scope','$state','$timeout','$http','$location','$interval',function($rootScope,$scope,$state,$timeout,$http,$location,$interval) {
...
//	//根据登录的用户查询用户信息
	...
	$http.get(spaceJeucIp+"/jeuc/api/ea/eaSpacePhoto/photoList?"+params).success(function (data){
		if(data.ret==200){
	...
	$scope.deleteAlbum = function (index,photoId){
		...
		$scope.sureDelete = function (){
			...
			console.log(params);
			$http.get(spaceJeucIp+"/jeuc/api/ea/eaSpacePhoto/deletePhoto?"+params).success(function (data){
			if(data.ret==200){
