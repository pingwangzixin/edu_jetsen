app.controller('standardAddCtrl',['$scope','$state','$stateParams','$timeout','$http','$location','$interval',function($scope,$state,$stateParams,$timeout,$http,$location,$interval) {
    ...
    function findSubject(gradeName){
    	...
    	if(gradeName=="校本课程"){
    		$http.post(suzhiIp + '/EvaluationManagerContr/getXiaoBen.do', {
				...
				console.info(data);
				if(data != null && data != ""){
					$scope.variablePacket.standard.arrSubject=data.data;
				}
			});
	...
	$scope.addStandardBtn = function (){
		...
		if($scope.userRole=="市领导"){
			...
			if($scope.variablePacket.standard.defaultGrade=="校本课程"){
				zong = selectGrade+"("+selectTerm+")"+selectSubject+selectStandard;
			}else{
	...
	//添加标准提交按钮
	$scope.addStandardBtnShi = function (){
		...
		var zong = selectGrade+"("+selectTerm+")"+selectSubject+selectStandard;
        	...
        	$http({
            	method: 'POST',
            	url: suzhiIp + 					'EvaluationManagerContr/addExcel.do?subject='+encodeURI(encodeURI(selectSubject)								)					+'&zong='+encodeURI(encodeURI(zong)),
            	data: form,
           		headers: {'Content-Type': undefined},
            	transformRequest: angular.identity
        	}).success(function (data) {
