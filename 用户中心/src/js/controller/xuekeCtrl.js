app.controller('xuekeCtrl',['$scope','$timeout','xuekeService','$http','$state',function($scope,$timeout,xuekeService,$http,$state) {
    ...
    $scope.changeLevel=function(m,n){
        ...
        $scope.getSubList(n.id,null);
    	$scope.getGradeBylevelId(n.id,null);
    }
    ...
    $scope.searchSchool = function(){
    	...
    	$scope.getSubTypeListByOfficeId(null,$scope.state.officeId);
    	$scope.getGradeSubListByOfficeId(null,$scope.state.officeId);
    }
    
    // 2.通过学段 获取 学科分类集合
	$scope.getSubList = function(levelId,officeId) {
		var params = {levelId:levelId,officeId:officeId}
     ...
     $scope.deltc = function (event,id) {
             ...
             $scope.suredel = function () {
                 ...
                 if ($scope.flag == false) {
                 	$http.post(requireIp + '/edu/eduSubject/deleteSubjectByType',{id:id,levelId:$scope.state.levelId}
	                 ).success(function (res) {
	                 	if(res.ret == '200') {
	                 		 $scope.getSubList($scope.state.levelId,null);
	                 		 $scope.getGradeBylevelId($scope.state.levelId,null);
	                 		 $(".zy_warningBox").show();
     ...
     $scope.tjbanji = function (event,subType,xuekelist) {
        ...
        if(!$scope.state.schoolshowstate) {
         ...
         $scope.baocun = function (event,type) {
         	...
         	// 3.查询 学校所属的学段 的学科
         	var params = {officeId:$scope.state.officeId,type:$scope.state.subType}
     ...
     $scope.bianjiroleclick=function(n,sort,subject){
     	if(!$scope.state.schoolshowstate) {
	         ...
	         for(var i=0;i<subject.length;i++){
	      	...
	      	//设置 查询 年级学科 
	      		xuekeService.getSubjectList(levelId,function(res){
					if(res.ret == 200){
						var temList = res.data;
						for(var i=0;i<temList.length;i++){
							if(typeof(temMap[temList[i].id])!='undefined')
	         ...
	         for(var i=0;i<subject.length;i++){
	      	...
	      	//设置 查询 年级学科 
	      		xuekeService.getOfficeSubjectList(officeId,function(res){
					if(res.ret == 200){
						var temList = res.data;
						for(var i=0;i<temList.length;i++){
							if(typeof(temMap[temList[i].id])!='undefined')
    ...
    $scope.sureadd = function(target) {
    	...
    	if($scope.state.tkstate){
			if(sidList != "") {
				if (!$scope.state.schoolshowstate) {
		    		var params = {levelId:$scope.state.levelId,gradId:$scope.state.gradId,sidList}
    ...
    // 4.获取 设置学段和年级的 学科
    $scope.getGradeBylevelId =  function(levelId,officeId){
		var params = {levelId:levelId,officeId:officeId}
    ...
    // 1.通过学校的ID 获取 学科分类集合
	$scope.getSubTypeListByOfficeId = function(levelId,officeId) {
		var params = {levelId:levelId,officeId:officeId}
	...
	// 2.通过学校的ID 获取 年级学科集合
	$scope.getGradeSubListByOfficeId = function(levelId,officeId) {
		var params = {levelId:levelId,officeId:officeId}
