...
app.service('xuekeService',['$http','$timeout',function($http,$timeout) {
	...
	// 3.获取学科 根据学段ID
	this.getSubjectList = function(levelId,succ,error) {
		$http.post(requireIp+'/edu/eduSubject/getSubListByLevelId',{levelId:levelId})
		.success(function(res) {
			succ(res)
		})
		.error(function(e) {
	...
	// 6.查询学校所有的学科
	this.getOfficeSubjectList = function(officeId,succ,error) {
		$http.post(requireIp+'/edu/eduSubject/getOfficeSubjectList',{officeId:officeId})
		.success(function(res) {
			succ(res)
		})
		.error(function(e) {
