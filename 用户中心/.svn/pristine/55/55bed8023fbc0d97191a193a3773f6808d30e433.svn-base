app.controller('jigouHandleCtrl',['$scope','$timeout','$filter','loginService','$http','$state',function($scope,$timeout,$filter,loginService,$http,$state) {
    ...
    function success(){
		...
		$scope.updateSchoolMsg = function(data,flag,state){
			$state.go('teacher_index.editorJigou',{officeId:data,flag:flag,state:state});
		}
		...
		if(sessionStorage.getItem("scope") == 2){
		    ...
		    $scope.addParentInfo = function(){
				...
				$http.post(requireIp + '/ea/eaArea/findAreaListByAreaId',{areaId:areaIdGrade}).success(function (data){
					console.log(data);
	    	...
	    	$scope.addParentInfo = function(){
	    		$scope.state.AddState = true;
		    	$http.post(requireIp + '/ea/eaArea/getAreaByAreaId',{areaId:areaIdGrade}).success(function (data){
	        		if(data.ret==200){
	        			$scope.areaa = data.data;
	        		}else{
	        			$scope.areaa = "";
		...
		$scope.submitOfficeMsg = function(){
			...
			if(isQuery){
				...
				$http.post(requireIp + '/ea/eaOffice/saveOfficeAndGrade',{name:$scope.officeName,type:'1',areaId:$scope.parentsList.areaId,grade:$scope.officeType}).success(function (data){
					$scope.state.AddState = false;
					console.log(data);
					if(data.ret == "200"){
						$scope.state.AddState = false;
	    ...
	    $scope.suredel = function(flags){
			...
			if(flags==3){
				$http.post(requireIp + '/ea/eaOffice/delAndrRestoreOffice',{ids:$scope.parentsList.checkboxArr.join(','),flag:'3'}).success(function (data){
					 console.log(data);
					if(data.ret == "200"){
						$scope.state.AddState = false;
						$scope.state.warningShow = true;
				...
				$http.post(requireIp + '/ea/eaOffice/delAndrRestoreOffice',{ids:$scope.parentsList.checkboxStopArr.join(','),flag:'3'}).success(function (data){
					 console.log(data);
					if(data.ret == "200"){
						$scope.state.AddState = false;
						$scope.state.warningShow = true;
				...
				$http.post(requireIp + '/ea/eaOffice/delAndrRestoreOffice',{ids:$scope.parentsList.checkboxReArr.join(','),flag:'1'}).success(function (data){
					 console.log(data);
					 if(data.ret == "200"){
						$scope.state.AddState = false;
						$scope.state.warningShow = true;
	    ...
	    $scope.parentonlineAction = function(state){
	    	if(!$scope.parentsList.checkboxArr.length){
				...
				console.info($scope.parentsList.checkboxArr.join(','))
				 $http.post(requireIp + '/ea/eaOffice/changeOfficeMsg',{ids:$scope.parentsList.checkboxArr.join(','),state:'2'}).success(function (data){
					 console.log(data);
					 //$scope.areaa = data.data;
					 if(data.ret == "200"){
						$scope.state.AddState = false;
	    ...
	    $scope.parentStopAction = function(state){
	    	if(!$scope.parentsList.checkboxStopArr.length){
				...
				console.info($scope.parentsList.checkboxStopArr.join(','))
				 $http.post(requireIp + '/ea/eaOffice/changeOfficeMsg',{ids:$scope.parentsList.checkboxStopArr.join(','),state:'1'}).success(function (data){
					 console.log(data);
					if(data.ret == "200"){
						$scope.state.AddState = false;
						$scope.state.warningShow = true;
	    ...
	    $scope.user=function(){
	    	if(sessionStorage.getItem("tableChangeP")==0){
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'0',state:'1',name:'',areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize}).success(function (data){
					console.log(data);
					if(data.ret == 200){
				...
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'0',state:'2',name:$scope.state.parentStopSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize}).success(function (data){
					console.log(data);
					if(data.ret == 200){
				...
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'3',state:'',name:$scope.state.parentRecoverSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize}).success(function (data){
					console.log(data);
					if(data.ret == 200){
	...
	//          onlineName.push(v.prarentsName);
	    ...
	    $scope.parentRecoverAction = function(change){
	        if(!$scope.parentsList.checkboxReArr.length){
				...
				$http.post(requireIp + '/ea/eaOffice/delAndrRestoreOffice',{ids:$scope.parentsList.checkboxReArr.join(','),flag:'0'}).success(function (data){
					 console.log(data);
					 if(data.ret == "200"){
						$scope.state.AddState = false;
						$scope.state.warningShow = true;
	    ...
	    $scope.onkeydown = function(data){
	    	if(data=="online"){
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'0',state:'1',name:$scope.state.parentOnlineSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade}).success(function (data){
					console.log(data);
					if(data.ret == 200){
						$scope.parentsList.tableMsgList = data.data.schoolList;
						$scope.state.parentsOnlineCount =data.data.schoolCount;
				...
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'0',state:'2',name:$scope.state.parentStopSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade}).success(function (data){
					console.log(data);
					if(data.ret == 200){
						$scope.parentsList.tableMsgStop = data.data.schoolList;
						$scope.state.parentsStopCount =data.data.schoolCount;
				...
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'3',state:'',name:$scope.state.parentRecoverSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade}).success(function (data){
					console.log(data);
					if(data.ret == 200){
						$scope.parentsList.tableMsgListRecover = data.data.schoolList;
						$scope.state.parentsRecoverCount =data.data.schoolCount;
	    ...
	    $scope.handleSearch = function(data) {
			if(data=="online"){
//				$scope.state.parentOnlineSearch=data;
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'0',state:'1',name:$scope.state.parentOnlineSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade}).success(function (data){
					console.log(data);
...
//				$scope.state.parentStopSearch=data;
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'0',state:'2',name:$scope.state.parentStopSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade}).success(function (data){
					console.log(data);
					if(data.ret == 200){
						$scope.parentsList.tableMsgStop = data.data.schoolList;
...
//				$scope.state.parentRecoverSearch=data;
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'3',state:'',name:$scope.state.parentRecoverSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade}).success(function (data){
					console.log(data);
					if(data.ret == 200){
						$scope.parentsList.tableMsgListRecover = data.data.schoolList;
...
//	         totalItems: 100  ,
	        ...
	        onChange:function() {
				...
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'0',state:'1',name:$scope.state.parentOnlineSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade,name:$scope.state.parentOnlineSearch}).success(function (data){
					console.log(data);
					if(data.ret == 200){
						$scope.parentsList.tableMsgList = data.data.schoolList;
						$scope.state.parentsOnlineCount =data.data.schoolCount;
...
//	         totalItems: 100  ,
	        ...
	        onChange:function() {
	        	var currentPage = this.currentPage;
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'0',state:'2',name:$scope.state.parentStopSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade,name:$scope.state.parentStopSearch}).success(function (data){
					console.log(data);
					if(data.ret == 200){
						$scope.parentsList.tableMsgStop = data.data.schoolList;
						$scope.state.parentsStopCount =data.data.schoolCount;
...
//	        totalItems: 100  ,
	        ...
	        onChange:function() {
	        	var currentPage = this.currentPage;
				$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'3',state:'',name:$scope.state.parentRecoverSearch,areaIds : areaIdGrade,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade,name:$scope.state.parentRecoverSearch}).success(function (data){
					console.log(data);
					if(data.ret == 200){
						$scope.parentsList.tableMsgListRecover = data.data.schoolList;
						$scope.state.parentsRecoverCount =data.data.schoolCount;
		...
		$scope.selettypefn = function(areaId,schoolGrade){
			...
			$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'0',state:'1',name:$scope.state.parentOnlineSearch,areaIds :$scope.parentsList.areaId, pageNo:currentPage, pageSize:pageSize,grade: $scope.parentsList.schoolGrade}).success(function (data){
				console.log(data);
				if(data.ret == 200){
					$scope.parentsList.tableMsgList = data.data.schoolList;
					$scope.state.parentsOnlineCount =data.data.schoolCount;
			...
			$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'0',state:'2',name:$scope.state.parentStopSearch,areaIds : $scope.parentsList.areaId,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade}).success(function (data){
				console.log(data);
				if(data.ret == 200){
					$scope.parentsList.tableMsgStop = data.data.schoolList;
					$scope.state.parentsStopCount =data.data.schoolCount;
			...
			$http.post(requireIp + '/ea/eaOffice/findSchoolList',{flag:'3',state:'',name:$scope.state.parentRecoverSearch,areaIds : $scope.parentsList.areaId,pageNo:currentPage,pageSize:pageSize,grade: $scope.parentsList.schoolGrade}).success(function (data){
				console.log(data);
				if(data.ret == 200){
					$scope.parentsList.tableMsgListRecover = data.data.schoolList;
					$scope.state.parentsRecoverCount =data.data.schoolCount;
