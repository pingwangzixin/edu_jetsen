...
//	console.log($rootScope.comeBackBtn)
...
app.directive('header',function (){
 	return {
 		restrict : 'E',
 		template : '<div class="zy_header"><div class="zy_navBg"><div class="zy_nav clearfix"><img  class="fl zy_jetsen" src="img/logo_white.png"/><a class="zy_back fr" href="http://111.207.13.88:8883/#/wrap/index">返回云空间首页<i></i></a></div></div></div>',
 		link : function (){
        ...
        link: function(scope, element, attrs) {
        	...
        	if(sessionStorage.getItem("scope") == 2){
        		...
        		$http.post(requireIp + '/uc/ucUser/findUserInfoUserId',{userId:sessionStorage.getItem("userId"),userType:sessionStorage.getItem("userType")}).success(function (dataUser){
	        		if(dataUser.ret==200){
	        			console.info(dataUser.data.userInfo.areaId)
...
//		    	if(previousId != undefined){
			        ...
			        if(scope.isjigou){
			        	...
			        	$http.post(requireIp + '/ea/eaOffice/findSchoolInfoByAreaId',{areaId : scope.changeActive,grade:item.id,state:'1',flag:'0'}).success(function (data){
			        		scope.map.clickstate = false;
			        		scope.map.resultstate = true;
			        		loadingfn(false);
			        		if(data.ret == 200){
