app.controller('resourcesIndexCtrl',['$scope','$state','$timeout','$http','$location',function($scope,$state,$timeout,$http,$location) {
	...
	$http.get(resourcesIp + "edu-resource/a/resource/mrs_rmi/getResources?token=29B5DF07F7FC514807CE5FBC12EA1506&order=1&pageSize=6&indexType=2&state=3").success(function(data) {
       $scope.newResource = data.result.list;
       console.log("最新资源");
       console.log(data.result.list);
	});
	
	$http.get(resourcesIp + "edu-resource/a/resource/mrs_rmi/getResources?token=29B5DF07F7FC514807CE5FBC12EA1506&order=1&type=1&pageSize=10&indexType=2&state=3").success(function(data) {
