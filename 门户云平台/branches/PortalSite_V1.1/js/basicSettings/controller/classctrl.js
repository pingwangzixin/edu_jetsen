app.controller('classctrl',['$scope','$state','$timeout','$http','$location','$interval','templateServer',function($scope,$state,$timeout,$http,$location,$interval,templateServer) {
	...
	$http.post(jeucIp+'ea/eaClass/findClassAndGradeByOfficeId',{officeId:$scope.officeId}).success(function(res){
		$scope.xuexiao = res;
	})
	...
	//鼠标移入班级出现叉号
     ...
     $scope.deltc = function (event) {
             ...
             $scope.suredel = function () {
                 ...
                 $http.post(jeucIps+'jdz-jeuc/api/ea/eaClass/delClassById', {
                     id: $scope.delbjid
     ...
     $scope.tjbanji = function (n) {
         $scope.addClass=true;
         $scope.gradeId = n;
         $http.post(jeucIp+'ea/eaClass/findClassInfoByGid', {
             gradeId: n
         }).success(function (res) {
     ...
     $scope.sureadd = function () {
         ...
         $http.post(jeucIps+'jdz-jeuc/api/ea/eaClass/insertClassInGrade', {
             gradeId: $scope.gradeId,
             className:className
         }).success(function (res) {
             $scope.addClass=false;
             if(res.ret == 200){
             	$http.post(jeucIps+'jdz-jeuc/api/ea/eaClass/findClassAndGradeByOfficeId',{officeId:$scope.officeId}).success(function(res){
					$scope.xuexiao = res;
				})
                 $(".zy_warningBox").show();
             ...
             $http.post(jeucIps+'jdz-jeuc/api/ea/eaClass/findClassAndGradeByOfficeId', schoolId).success(function (res) {
                 $scope.xuexiao = res;
             })
         })
