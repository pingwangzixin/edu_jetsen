		...
		if(data.ret==200){
			...
			$http.get(jeucIp+"/ea/office/"+sessionStorage.getItem("officeId"))
				.success(function (officeData) {
					...
					$scope.officeName = officeData.name;sessionStorage.getItem("officeId")==undefined
				}
			})
		}
	});
	    	...
	    	.success(function(res){
	    		if(res.ret==200){
					...
					$http.post(jeucIp + "/ea/eaSpaceIntroduce/updateBackGroundImage",params
				    	...
				    	if(data.ret ==200){
				    	sessionStorage.setItem("backGroundImagePath",$scope.commonVar.backGroundImagePath)	
				    		$state.reload();
