app.controller('publicResourceCtrl', ['$rootScope','$scope', '$state', '$stateParams', '$timeout', '$http', '$location', '$interval', 'templateServer', function($rootScope,$scope, $state, $stateParams, $timeout, $http, $location, $interval, templateServer) {
	//导航相关设置（导航名称等）
    ...
    $scope.findResourcePage = function() { //文件类型头部切换
        ...
        var title = $scope.variablePacket.title;objId,subjectID
        ...
        $http.get(resourcesIp+'a/resource?state=2&pageNo='+pageNo+'&pageSize=7&token='+token+"&title="+title+"&objId="+pobjId+"&subjectID="+subjectID+"&knowledge="+knowledge).success(function (data){
            if(data.code == 200){
    ...
    $scope.findResource = function(title,objId,subjectID,knowledge) { //文件类型头部切换
        ...
        $http.get(resourcesIp+'a/resource?state=2&pageNo='+pageNo+'&pageSize=7&token='+token+"&title="+title+"&objId="+objId+"&subjectID="+subjectID+"&knowledge="+knowledge).success(function (data){
            if(data.code == 200){
                $scope.variablePacket.total = data.data.count;
                var list = data.data.list;
                angular.forEach(list, function(data){
	...
	$scope.openStateTab = function(item, event) { //公开状态切换
		i.open = !i.open;
	};
