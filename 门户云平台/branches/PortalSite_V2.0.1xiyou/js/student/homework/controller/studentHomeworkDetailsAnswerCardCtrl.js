	...
	$scope.findStuExamList= function(){
		...
		$http.get(url).success(function(result) {
			if(result.ret==200){
				...
				$scope.variablePacket.examAssemblyName = examData.examAssembly.name;
				$scope.variablePacket.questionImgList = examData.examAssembly.questionImgList;
				var questionOptionMap = examData.examAssembly.questionOptionMap;
				for(var t=0,tlen=questionOptionMap.length; t<tlen; t++){
					...
					for(var i=0; i<questionList.length; i++){
						...
						if($scope.variablePacket.state=='unsubmitted'){
							...
							valueObject.myAnswer = checkMyAnswer(question.myAnswer, question.icon, key, question.answer, $scope.variablePacket.state)//我的答案--已学习选项
...
//			$state.go('secondNav.studentGuideList');
	...
	$scope.delOk = function(){
		...
		if(examData.examType>0){
			examAssemblyId = examData.examAssembly.id;
		}
		var questionArray = [];
		var questionObj = {};
		angular.forEach($scope.echoQuestion.single,function(e){
