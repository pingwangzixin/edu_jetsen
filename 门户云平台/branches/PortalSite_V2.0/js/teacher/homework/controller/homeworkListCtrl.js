    ...
    */
	...
	$scope.selectinexam = function (){
		...
		var url = lessonIp+"ExamCount/selectExam?subId=";
		 $http.get(url + subId+"&classId="+$scope.selectexam.classId+"&stateId="+$scope.variablePacket.typesstate+"&teacherId="+teacherId+"&type="+"0"+"&treeids="+$scope.resParams.subjectID+"&currentPage="+$scope.variablePacket.currentPage+"&numPerPage="+$scope.variablePacket.numPerPage).success(function (response) {
	    	var examjson = JSON.stringify(response);
	    	var examjsons = JSON.parse(examjson)
	    	var  state = "";
	    	if(examjsons.data!=undefined){
	...
	$scope.addParams = function(examId, examName, startTime, endTime, remark, state, type){
		var teaExamInfo = {};
		...
		$state.go('secondNav.homeworkDel',{state:state,type:type},{reload:true});
	}
