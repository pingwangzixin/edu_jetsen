...
//			console.log(activeArr)
	...
	$scope.Verification = function(ok){
		...
		//$scope.validForm();
		$timeout(function(){
			if(ok && (!$scope.variablePacket.ProvingUsername) && (!$scope.variablePacket.ProvingSubject) && ($scope.variablePacket.Chapter[0].Onetit!="课本" && $scope.variablePacket.Chapter[0].Twotit!="章" && $scope.variablePacket.Chapter[0].Threetit!="节") && (!$scope.variablePacket.ProvingExplain) &&  (!$scope.variablePacket.ProvingSex) &&  (!$scope.variablePacket.ProvingObject) && ( $scope.variablePacket.eightSwitchOut_show || $scope.variablePacket.AddResources_show)){
				var classNames = "";
				var classIds = "";
