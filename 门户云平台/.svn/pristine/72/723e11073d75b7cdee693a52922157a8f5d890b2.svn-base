...
//						qu.Answer = i.answer.replace(/<.*?>/ig,"");
						qu.Answer = i.answer;
					}
				
//					qu.Analysis = i.analysis.replace(/<.*?>/ig,"");
					qu.Analysis = i.analysis;
					qu.id = i.qid;
					qu.Id = r;
					qu.sign = true;
//					qu.queTit = i.body.replace(/<.*?>/ig,"");
					qu.queTit = i.body;
					qu.Chapter = [{
						"Onetit":"",
						"Threetit": "",
...
//					    json1.queTit=s.body.replace(/<.*?>/ig,"");
					    json1.queTit=s.body;
					    if(s.answer=="T"){
					    	json1.Answer="正确";
					    }else{
...
//					    	var b = s.analysis.replace(/<.*?>/ig,"");
					...
					angular.forEach(s.questions,function(e){
						...
						if(e.type=="2"){
							...
							swer.Type = 'material'; 
//							swer.Answer = e.answer.replace(/<.*?>/ig,"");
							swer.Answer = e.answer;
						}else if(e.type=="6"){
...
//						swer.Analysis = e.analysis.replace(/<.*?>/ig,"");
	...
	$scope.Charu = function(type, i,o) {
		...
		$scope.insert_Sign = function(index) {
			...
			if($scope.insertData[index].sign) {
				...
				if($scope.questionBank.Out[type][i].Resource==undefined){
			...
			$http.get(questionUrl+"exam/"+$scope.variablePacket.addresid).success(function(resJson){
				if(resJson.code == 200) {
					angular.forEach(resJson.data.questionList,function(quz,i) {
					    ...
					    json1.AnswerShow=false;
					    json1.ResourceShow=false;
//					    json1.queTit=quz.body.replace(/<.*?>/ig,"");
...
//					    json1.Analysis=quz.analysis.replace(/<.*?>/ig,"");
...
/*	//吸顶
	...
	$scope.insertChoice_eightSwitchTab = function(id,index) {
		...
		}else if($scope.variablePacket.statequ){
...
//				ques.queTit = e.body.replace(/<.*?>/ig,""); //题干
...
//					ques.Answer = e.answer.replace(/<.*?>/ig,""); //答案
					ques.Answer = e.answer; //答案
				}
				
//				ques.Analysis = e.analysis.replace(/<.*?>/ig,"");//解析
