...
//                  gid:getGradeNo(data.gradeName),
	...
	$scope.findPrepareList = function(){	
		...
		$http.get(prepareServerIp + '/prepare?teaId='+$scope.variablePacket.userId+'&chapterIds='+chapterIds+'&pageNo='+$scope.contentpageConfig.currentPage+"&pageSize="+$scope.contentpageConfig.itemsPerPage).success(function(suc) {
	        if(suc.ret == 200) {
	        	if(suc.total == 0){
	        		$scope.variablePacket.filetypeContent = [];
	        		$scope.variablePacket.noData = true;
	        		$scope.variablePacket.jiazai=false;
	        	}else{
	        		$scope.variablePacket.noData = false;
	        		$scope.variablePacket.jiazai=true;
	        		$scope.contentpageConfig.totalItems = suc.total;
		        	$scope.variablePacket.filetypeContent = [];
		        	if(suc.data){
	...
	$rootScope.others = function(thisCharterIds){
    	...
    	$scope.variablePacket.subjNames = "";
    	$scope.findPrepareList();
	}	
