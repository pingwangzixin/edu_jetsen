app.controller('publicQuestionBankCtrl',['$scope','$state','$timeout','$http','$location','$interval','templateServer','myQuzService','$rootScope',function($scope,$state,$timeout,$http,$location,$interval,templateServer,myQuzService,$rootScope) {
	...
	$scope.exercisesAndtestTab = function (i){
		...
		if(i==1){
			$scope.exams = [];
			$scope.findMyExamList($scope.variablePacket.pageNo,$scope.variablePacket.pageSize,$scope.variablePacket.subjIds);
		}else {
			$scope.getType($scope.variablePacket.type);
			$scope.findShareAndFavoutite($scope.variablePacket.pageNo,$scope.variablePacket.pageSize,$scope.variablePacket.type,$scope.variablePacket.subjIds);
		}
	...
	$rootScope.findResAll = function() {
		...
		$scope.exercisesAndtestTab($scope.variablePacket.exercisesAndtest);
	}
	...
	$scope.findQuzAll = function(pageNo,pageSize,type,subjIds) {
		var params = {pageNo:pageNo,pageSize:pageSize,type:type,subjIds:subjIds,state:2};
	...
	$scope.findShareAndFavoutite = function(pageNo,pageSize,type,subjIds) {
		...
		myQuzService.findShareAndFavoutite(params,function(res) {
			...
			// 第一次 调用quz全部展示默认单选
			$scope.findQuzAll($scope.variablePacket.pageNo,$scope.variablePacket.pageSize,$scope.variablePacket.type,$scope.variablePacket.subjIds)
			}
		},function(e){
...
//////////////////////////////////////////////////////////////////
	// 校本试卷
	$scope.findExamList = function(pageNo,pageSize,subjIds) {
		var params = {current:pageNo,size:pageSize,state:2,module:1,subjIds:subjIds};
	...
	// 我的试卷收藏
	$scope.findMyExamList = function(pageNo,pageSize,subjIds) {
		var params = {current:pageNo,size:pageSize,module:3,subjIds:subjIds,createBy:userId};
		myQuzService.getExamList(params,function(res) {
			if(res.code == 200) {
				myExamsfavMap = {};
				...
				$scope.findExamList($scope.variablePacket.pageNo,$scope.variablePacket.pageSize,$scope.variablePacket.subjIds);
			}
		},function(e) {
