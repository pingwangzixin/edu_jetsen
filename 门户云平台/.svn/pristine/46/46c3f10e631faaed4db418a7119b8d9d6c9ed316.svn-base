...
//				state : null,		//unsubmitted:未提交		submission:已提交		readyOver：已批阅
		...
		.state('secondNav.guideDelNew', { //导学详情/批阅导学/导学分析容父级页面
			url: "/guideDelNew",
			templateUrl: 'tpl/teacher/guidance/guideDelNew.html',
			controller: "guideDelNewCtrl",
			params:{
			...
			resolve: {
				deps: ["$ocLazyLoad", function($ocLazyLoad) {
					return $ocLazyLoad.load("js/teacher/guidance/controller/guideDelNewCtrl.js");
				}]
			}
		})	
		.state('secondNav.guideDelNew.guidanceDetails', { //导学详情
			url: "/guidanceDetails",
			templateUrl: 'tpl/teacher/guidance/guidanceDetails.html',
			controller: "guidanceDetailsCtrl",
			resolve: {
				deps: ["$ocLazyLoad", function($ocLazyLoad) {
					return $ocLazyLoad.load("js/teacher/guidance/controller/guidanceDetailsCtrl.js");
				}]
			}
		})	
		.state('secondNav.guideDelNew.markingGuidance', { //批阅导学
			url: "/markingGuidance",
			templateUrl: 'tpl/teacher/guidance/markingGuidance.html',
			controller: "markingGuidanceCtrl",
			resolve: {
				deps: ["$ocLazyLoad", function($ocLazyLoad) {
					return $ocLazyLoad.load("js/teacher/guidance/controller/markingGuidanceCtrl.js");
				}]
			}
		})	
		.state('secondNav.guideDelNew.guidanceAnalysis', { //导学分析
			url: "/guidanceAnalysis",
			templateUrl: 'tpl/teacher/guidance/guidanceAnalysis.html',
			controller: "guidanceAnalysisCtrl",
			resolve: {
				deps: ["$ocLazyLoad", function($ocLazyLoad) {
					return $ocLazyLoad.load("js/teacher/guidance/controller/guidanceAnalysisCtrl.js");
				}]
			}
		})	
		.state('secondNav.guideDelNew.completionStatus', { //完成情况
			url: "/completionStatus",
			templateUrl: 'tpl/teacher/guidance/completionStatus.html',
			controller: "completionStatusCtrl",
			resolve: {
				deps: ["$ocLazyLoad", function($ocLazyLoad) {
					return $ocLazyLoad.load("js/teacher/guidance/controller/completionStatusCtrl.js");
				}]
			}
		})	
		//教师端导学结束
...
//			console.log(scope.subJectId)
            $rootScope.initchoiceVersion = function(){
                scope.p = $rootScope.variableGlobal.libraryTitleBar[0];
                scope.c = $rootScope.variableGlobal.libraryTitleBar[1];
                scope.d = $rootScope.variableGlobal.libraryTitleBar[2];
                scope.titleShow = false;
                scope.directiveState = false;
            }
			//监听页面版本回显
