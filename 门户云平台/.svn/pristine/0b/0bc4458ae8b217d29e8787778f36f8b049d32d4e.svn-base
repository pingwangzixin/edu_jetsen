app.controller('studentUploadResourceCtrl', ['$rootScope','$scope', '$state', '$stateParams', '$timeout', '$http', '$location', '$interval', 'templateServer', function($rootScope,$scope, $state, $stateParams, $timeout, $http, $location, $interval, templateServer) {
	//导航相关设置（导航名称等）
    ...
    $http.get(zyxrequireIp + '/uc/user/'+$scope.variablePacket.userId).success(function(suc) {
        if(suc.ret == 200) {
            ...
            $scope.variablePacket.gradeId=suc.data.gradeId;
            $scope.variablePacket.classId=suc.data.classId;
...
//				console.log(data.data);
                $scope.variablePacket.selectSubject=data.data;
...
//				初始化的学科
    ...
    $scope.verifySubject=function(){    //点击章节目录时先验证学科
     ...
     $scope.verifyName=function(){    //验证资源名字

        if($scope.variablePacket.resourceName){
    ...
    $scope.clearAll=function(){    //点击取消
    	$scope.selectedSubject=undefined;
    	angular.element('.titleSpan').find('em')[0].innerHTML="课本";
    	angular.element('.titleSpan').find('em')[1].innerHTML="章";
    	angular.element('.titleSpan').find('em')[2].innerHTML="节";
    	$scope.variablePacket.resourceName="";
        $scope.variablePacket.keywords="";
    	$scope.variablePacket.ProvingSubject=false;
    	$scope.variablePacket.ProvingChapter=false;
    	$scope.variablePacket.ProvingName=false;
    	$("#theList").empty();
	...
	uploader.on("fileQueued", function(file) {
		$("#wenben").attr("style","display:none;")		// 上传成功后 隐藏把文件拖拽到这里
        $(".uploadFile").attr("style","display:none;")		// 上传成功后 隐藏加号
...
//	});
	...
	$("#theList").on("click", ".itemDel", function() {
		...
		$(".uploadFile").attr("style","display:block;")				// 删除后显示加号
		$("#wenben").attr("style","display:block;")				// 上传成功后 显示把文件拖拽到这里
	});
