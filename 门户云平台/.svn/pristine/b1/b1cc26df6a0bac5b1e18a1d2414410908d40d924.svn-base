app.controller('schoolPhotoDetailsCtrl',['$rootScope','$scope','$state','$timeout','$http','$location','$interval','templateServer',function($rootScope,$scope,$state,$timeout,$http,$location,$interval,templateServer) {
	...
	$http.get(jeucIp+"/ea/eaSpacePhoto/photoDetails?"+params)
	...
	$scope.albumEvent = function (){
		...
		$scope.toRight = function (){
			...
			if($scope.variable.BimgIndex >= $scope.album.length - 1) $scope.variable.BimgIndex = $scope.album.length - 1;
			bigPic();
		};
		//照片大图向左
		$scope.toLeft = function (){
			$scope.variable.BimgIndex -- ;
			if($scope.variable.BimgIndex <= 0) $scope.variable.BimgIndex = 0;
			bigPic();
		};
