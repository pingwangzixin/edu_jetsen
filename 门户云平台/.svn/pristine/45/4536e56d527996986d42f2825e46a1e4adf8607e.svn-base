app.controller('studentHomeworkListCtrl',['$rootScope','$scope','$state','$timeout','$stateParams','$http','$location','$interval','templateServer','scrollbar','$sce','$compile',function($rootScope,$scope,$state,$timeout,$stateParams,$http,$location,$interval,templateServer,scrollbar,$sce,$compile) {
	//导航相关设置（导航名称等）
	...
	$scope.variablePacket = {
		...
		curr : 0 , //初始化当前状态
		states:[{id:0,name:'未提交',state:'unsubmitted'},{id:1,name:'未批阅',state:'submission'},{id:2,name:'已批阅',state:'readyOver'}],
		stateIndex:0, //状态索引
		...
		classify : [{name:'课中作业'},{name:'课后作业'}], // 选择类型
...
//	var stuId = '956ff099a4ec463f9271a5df3aacba28';
	...
	$scope.findStuExamList = function(flag){
		var eType = 0;
		if($scope.variablePacket.classifyIndex==0){
			eType = 1;
		}
		var url = lessonIp+"stuExam/findStuExamList?eType="+eType+"&type=0&stuId="+$scope.variablePacket.stuId
		...
		$http.get(url).success(function(result) {
			if(result.ret==200 && result.total > 0){
				var data = result.data;
				var examObj = {};
				for (var i=0; i<data.length; i++) {
	...
	$scope.addParams = function(examId, url, type, state, stage, subjectName){
		var stuExamInfo = {};
		...
		param.type = 'homework';
		param.stage = stage;
		param.packjectId = examId;
		param.classId = $scope.variablePacket.classId;
		param.className = $scope.variablePacket.className;
