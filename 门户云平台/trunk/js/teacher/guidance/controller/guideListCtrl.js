app.controller('guideListCtrl',['$rootScope','$scope','$state','$stateParams','$timeout','$http','$location','$interval','templateServer','scrollbar','$sce','$compile',function($rootScope,$scope,$state,$stateParams,$timeout,$http,$location,$interval,templateServer,scrollbar,$sce,$compile) {
	//导航相关设置（导航名称等）
	$rootScope.variableGlobal.navShowDet.modularName = '导学';
...
//	sessionStorage.setItem("loginUserId",$scope.variablePacket.loginUserId);
	...
	$scope.getGuidanceLearningList = function(subjectId,classId,pageSize) { //文件类型头部切换
		console.log($scope.variablePacket.classifyIndex)
		if($scope.variablePacket.classifyIndex == 1){
			$scope.variablePacket.guideType = 1;
		}else{
			$scope.variablePacket.guideType = 0;
		}
		$http.get(guidanceLearningIp + 'learn?memoriesChapterIds='+subjectId+"&&publishObjectIds="+classId+"&&pageSize="+pageSize+"&&rows="+$scope.contentpageConfig.itemsPerPage+"&&selectType="+$scope.variablePacket.selectType+"&&createBy="+$scope.variablePacket.loginUserId+"&&guideType="+$scope.variablePacket.guideType).success(function (data){
			console.log(data);
			if(data.ret == 200){
				...
				$scope.contentpageConfig.totalItems=data.data[0].total;
...
//				console.log($scope.variablePacket.userCourse)
	...
	$scope.selectClassifyTab = function(n){
		...
		console.log($scope.variablePacket.guideType);
		$scope.getGuidanceLearningList($scope.variablePacket.subjectID,$scope.classSummary.classId,1);
	};
	//布置导学弹框
	...
	$scope.jiazai = function(pageSize){
		...
		//不用年级筛选
		$http.get(guidanceLearningIp + 'learn?memoriesChapterIds='+$scope.variablePacket.subjectID+"&&publishObjectIds="+$scope.classSummary.classId+"&&pageSize="+pageSize+"&&selectType="+$scope.variablePacket.selectType+"&&createBy="+$scope.variablePacket.loginUserId).success(function (data){
			console.log(data);
			if(data.ret == 200){
				$scope.variablePacket.jiazai=true;
...
//				}
	...
	$scope.contentpageConfig={
		...
		totalItems:0,
		onChange: function() {
			$scope.getGuidanceLearningList($scope.variablePacket.subjectID,$scope.classSummary.classId,this.currentPage);
		}
	};
	/**
	 * 点击other
