app.controller('paperLibraryCtrl', ['$rootScope', '$scope', '$state', '$timeout', '$http', '$location', '$anchorScroll', '$interval', 'templateServer', 'scrollbar', '$stateParams', 'myQuzService', 'myResourceService', function($rootScope, $scope, $state, $timeout, $http, $location, $anchorScroll, $interval, templateServer, scrollbar, $stateParams, myQuzService, myResourceService) {
	...
	$scope.variablePacket = {
		...
		testPaper : 0,		//0 课中卷/1 PC卷切换
		inClassQues : {              //课中试卷习题对象
						single: [],
						judge: [],
						briefAnswer: []
					  },
	...
	//课中卷/PC卷切换
	$scope.testPaperTab = function (i){
		if($scope.variablePacket.testPaper != i){
			var sourceTypeNames = 'pcQues';
			var targetTypeNames = 'inClassQues';
			if(i == 1){
				sourceTypeNames = 'inClassQues';
				targetTypeNames = 'pcQues';
			}
			$scope.variablePacket[sourceTypeNames] = angular.copy($scope.questionBank.Out);
			$scope.questionBank.Out = angular.copy($scope.variablePacket[targetTypeNames]);
			var typeMapTem = {};
			$scope.variablePacket.queSelMap = {};
			angular.forEach($scope.questionBank.Out,function(value,key){
				if(value.length>0){
					typeMapTem[key] = key;
					angular.forEach(value,function(que,queIndex){
						$scope.variablePacket.queSelMap[que.Id] = true;
					})
				}
			})
			angular.forEach($scope.variablePacket.eightSwitchOut,function(item,index){
				item.typeShow = true;
				item.show = true;
				if(i == 0 && $scope.variablePacket.typeNames.indexOf(item.name) == -1){
					item.typeShow = false;
				}
				if(typeMapTem[item.Type]==undefined){
					item.show = false;
				}
			})
		}
		$scope.variablePacket.testPaper = i;
	};
	...
	$scope.Verification = function() {
		if((!$scope.variablePacket.ProvingSubject) && ($scope.params.subjIds != "") && (!$scope.variablePacket.ProvingUsername)) {
			...
			$scope.params.type = $scope.variablePacket.testPaper == 0?'01':'0';
		...
		myQuzService.findQuzType(function(res) {
			if(res.code == 200) {
				angular.forEach(res.data, function(data) {
					if($scope.variablePacket.typeNames.indexOf(data.name) != -1){
						data.typeShow = true;
					}else{
						data.typeShow = false;
					}
			...
			myResourceService.getResourcesAll($scope.resParams, function(res) {
				...
				angular.forEach(res.list, function(obj, index) {
					...
					if(obj.sourceType == 1) {
						...
						resTem.ossFileName = obj.resourceId;
					}
	...
	$scope.findExamById = function(){
		myQuzService.getExamById($scope.variablePacket.examId,function(resJson){
			if(resJson.code == 200){
				...
				$scope.variablePacket.testPaper = data.type == '01'?0:1;
				var typeTabName = data.type == '01'?'inClassQues':'pcQues';
				var queTypeMap = {}
		        ...
		        $scope.variablePacket[typeTabName] = angular.copy($scope.questionBank.Out);
				angular.forEach($scope.variablePacket.eightSwitchOut,function(item,index){
					if(queTypeMap[item.id]){
						item.show = true;
					}
					if(data.type == '01' && $scope.variablePacket.typeNames.indexOf(item.name)==-1){
						item.typeShow = false;
					}
				})
