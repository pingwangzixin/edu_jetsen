...
$rootScope.findListByTree = function(subjectID,knowledge) {
	$scope.variablePacket.zjpageSize=4;
	$scope.variablePacket.sjpageSizes=4;
	...
	$scope.Release = function(){
		...
		angular.forEach(indexFlags,function(e,i){
			...
			angular.forEach($scope.questionBank.Out[tixing],function(ee,ii){
				...
				if(ee.Resource!=undefined&&ee.Resource.length!=0){
		...
		if($scope.variablePacket.addresid!=null&&$scope.variablePacket.addresid!=""&&$scope.variablePacket.addresid!=undefined){
			quessorts = [];
		}
		...
		,teacherId:teacherId,content:content,examType: typeex,addresid:asid,state:state,eType:0};
...
//			}
	...
	//资源列条上的删除
	$scope.delResource = function(tit, type, index,id) {
		$scope.promptShow('确认删除？', false, tit);
		$scope.delOk = function() {
			$scope.variablePacket.myziid = "";
			$scope.variablePacket.prompt = false;
			
			angular.forEach($scope.questionBank.Out[type],function(i,r){
				if(i.id==id){
					$scope.questionBank.Out[type][r].CanResource = false;
					$scope.questionBank.Out[type][r].Resource = [];
					$scope.questionBank.In[r].CanResource = false;
				}
			})
...
/*	//吸顶
	...
	$scope.selecSj = function() {
		$scope.variablePacket.sjpageSizes= $scope.variablePacket.sjpageSizes+4;
		$scope.exempng();
	}
	//资源加载更多
	$scope.selecZy = function() {
		$scope.variablePacket.zjpageSize= $scope.variablePacket.zjpageSize+4;
		$scope.insertzy();
	}
	//取消白块
	$scope.variablePacket.eightSwitchOut_show = false;
