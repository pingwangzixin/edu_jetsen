...
//	$scope.resRea = JSON.parse(sessionStorage.getItem('resRea'));
	...
	$scope.clickLevel = function(tar, level) {
		...
		if("AllSchool" != level){
			...
			$http.get(requireIp + 'jeuc/api/edu/eduSubject?leveId='+$scope.queryParam.levelId+"&areaId=130600").success(function(res) {
				console.log(res);
	...
	//资源格式
	$scope.resType;
	$http.get(resourcesIp_bd + 'edu-resource/a/resource/mrs_rmi/getTypesByPid?pid=070a33c388f24f23b05d15adc0b8fd2e&token=29B5DF07F7FC514807CE5FBC12EA1506').success(function(res) {
		console.log(res);
	...
	$scope.clickSub = function(tar, subjectId) {
		...
		$http.get(requireIp + 'jeuc/api/edu/eduVersion?subjectId=' + subjectId + "&levelId=" + $scope.queryParam.levelId+"&areaId=130600").success(function(res) {
			console.log(res);
	...
	$scope.toResourcesDetails = function(res) {
		console.log(res);
		/*var url = resourcesIp_bd + "/edu-resource/a/resource/mrs_rmi/getById?token=29B5DF07F7FC514807CE5FBC12EA1506&id=" + res.id;
