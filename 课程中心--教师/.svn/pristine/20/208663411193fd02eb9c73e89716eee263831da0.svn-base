...
$(document).ready(function(){
	...
	//任务列表新增云课堂
	$(".investment_title div").on("click",function(){
		 var index=$(this).index();
		 $(this).addClass("on").siblings().removeClass("on");
		 $(".investment_con_one").eq(index).addClass("investment_one").siblings().removeClass("investment_one");
		 
	});
	
	//作业列表----全选事件      //
	$('.list_table .all').on('click',function(){
		var parent=$(this).parents(".investment_con_one");
		var CheckBox =parent.find('.list_table tbody input');
		var gouxuan = parent.find('.list_table .all input[type=checkbox]').is(':checked');
		if(gouxuan){
            for(i=0;i<CheckBox.length;i++){
                CheckBox[i].checked=true;
            };
      	}else{
        	parent.find('.list_table .all input').prop('checked',false);
            for(i=0;i<CheckBox.length;i++){
	...
	$( '.list_table tbody input' ).on('click' , function(){
		var parent=$(this).parents(".investment_con_one");
        if (! parent.find( '.list_table tbody input').checked) {
           parent.find( '.list_table .all input[type=checkbox]'). attr ("checked" , false);
        }
        var sub_inpu_length =parent.find( '.list_table tbody input').size();
        var checkedsub = parent.find( ".list_table tbody input:checked").length; 
        if (checkedsub == sub_inpu_length) {
           parent.find( '.list_table .all input[type=checkbox]').prop( "checked" , true );
        }
   	});
  /*  新添纠错功能   2018-05-03  start*/
 $(".grade .em4").on("click",function(){
 	 $(".correct").show();
 	 
 });
 $(".errorSure").on("click",function(){
 
 	 var textVal=$.trim($(".describe_content").val());
...
/*  新添纠错功能   2018-05-03  end*/

/*新添批量公开功能   2018-05-03*/
$("#allCheckbox").click(function(){
	var  gouxuan=$(this).prop("checked");
	for(var i=0;i<$(".Z-otherTypes .question").length;i++){
		$(".Z-otherTypes .question").eq(i).find(".tit :checkbox").prop("checked",gouxuan);
	};
	
	 
});
$(".tit :checkbox").on("click",function(){
...
/*  新添纠正提示   2018-05-03  start*/
$(".grade .em5").on("click",function(){
	$(".reminder").show(); 
});
...
/*  新添纠正提示   2018-05-03  end*/
/*创建试题放大缩小功能 2018-05-04 start */
$(".amplification").on("click",function(){
	 if(!$(this).hasClass("shrink")){
	 	 $(this).addClass("shrink").html("缩小放大");
	 	 $(".leftSide").css("width","100%").siblings().hide();
	 	
	 	 
	 }else{
	 	  $(this).removeClass("shrink").html("放大查看");
	 	   $(".leftSide").css("width","49.5%").siblings().show();
	 };
	...
	//作业列表的撤回点击事件
	$(".list_table tbody td:nth-last-child(2)").on('click',function(){
		$(".chehui").show();
	});
	...
	$('.right_box .right_tit span').on('click',function(){
		...
		console.log(texts)
		if(texts == '我的题库' ){
			$('.person,.right_box .create').show();
			$('.puble,.Colligate').hide();
		};
		if(texts == '公共题库'){
			$('.person,.right_box .create,.Colligate').hide();
			$('.puble').show();
		};
		if(texts == '综合类'){
			$('.person,.puble,.right_box .create').hide();
			$('.Colligate').show();
		};
	});
...
//	var nNum = 0;
	...
	$(".Z-genre span").click(function(){     //编辑作业 添加试题  zmj
		$(this).addClass("active").siblings().removeClass("active");
		if($(this).index()==1){
			 $("#tree").show();
			 $(".screening_grade select").hide();
		}else{
			  $("#tree").hide();
			  $(".screening_grade select").show();
		}
	...
	$('.YzoomClose').on('click',function (){
		$('.YtaskMask').hide();
		$('.mark_right').removeClass('YtoBig').css({'height':'auto'}).siblings('.YzoomClose').hide();
	...
	$('.YjobZoom').on('click',function (){
		$('.YjobMask').show();
		$('.YjobBig').addClass('YtoBig').find('.YzoomClose').show().siblings('.YjobZoom').hide();
	});
	//关闭放大
	$('.YzoomClose').on('click',function (){
		$('.YjobMask').hide();
		$('.YjobBig ').removeClass('YtoBig').find('.YzoomClose').hide().siblings('.YjobZoom').show();
	});
	...
	LebalFor();
	function LebalFor(){
		$('.Zlines  ul input').each(function(index, element){
			$(this).attr("id","input"+ index +"");
		});
		$('.Zlines ul label').each(function(index, element){
	...
	$(".edit_task .task_types span").on('click',function(){
		...
		if(test == '综合'){
			$('.Z-synthetical').show();
			$('.edit_question,.previvw_task').hide();
		}else{
			$('.Z-synthetical').hide();
			$('.edit_question,.previvw_task').show();
		}
	});
	...
	//下发作业弹层
	$('.xiafa_btn').click(function(){
		$('.xiafa').show();
	});
	//左侧树
	$(document).ready(function(){
		    if($("#knowledgeZtree").length){
	...
	$(document).ready(function(){
		    if($("#knowledgeZtree20").length){
		    	$.fn.zTree.init($("#knowledgeZtree20"), settings, zNodes);
			    zTree_Menus = $.fn.zTree.getZTreeObj("knowledgeZtree20");
			    curMenus= zTree_Menus.getNodes()[0].children[0].children[0].children[0];//默认选中的
		    };
			
		    $("#knowledgeZtree20").on('click','.node_name',function(){
				$('#knowledgeZtree20 .node_name').parent().removeClass('curSelectedNode');
				$(this).parent().addClass('curSelectedNode');
