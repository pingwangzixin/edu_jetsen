...
function editClick(data){
	...
	if(data.style==2){
		...
		$.each(data.threeSubList, function(index,item) {
			...
			$('.sub_group_select').append('<span class="mlh_btn selected" data-ids="'+item.ids+'" data-names="'+item.names+'">'+item.name+'<i class="iconfont icon-ICon- del_sub_goup"></i></span>');
		});
...
function insertSub() {
	...
	if(data.style==2){
		var groupList = [];
		$('.sub_group_select span').each(function(i){
			var group = {};
		...
		if($('.sub_group_div .active').length==3){
			...
			$('.addsub_gruop_insert_btn').on('click',function(){
				...
				if(repeatGroup(groupId)){
					$('.sub_group_select').append('<span class="mlh_btn selected" data-ids="'+groupId+'" data-names="'+groupNames+'">'+groupName+'<i class="iconfont icon-ICon- del_sub_goup"></i></span>');
				}else{
					warnTips('已添加该组');
				}
...
function repeatGroup(groupId){
	var flag = true;
	$('.sub_group_select span').each(function(){
