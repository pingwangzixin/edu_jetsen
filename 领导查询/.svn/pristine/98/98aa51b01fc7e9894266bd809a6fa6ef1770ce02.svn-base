...
//	var managerSearch = JSON.parse(sessionStorage.getItem('managerSearch'))
	...
	//根据学校编号查询年级列表
	$http.get(jeucIp+"statistics/eaGrade?officeId="+schoolId).success(function(data){
		if(data.ret==200){
			var gradeListArray = [];
			angular.forEach(data.data,function(response,index){
				var gradeObj = {};
				gradeObj.gradeId = response.gradeId;
				gradeObj.gradeName = response.gradeName;
				gradeListArray.push(gradeObj);
			})
			$scope.gradeList = gradeListArray;
		}
	})
    if(scope=='2'){
    ...
    $scope.userTypeList = [
    	...
    	$scope.changeGrade= function(gradeId){
    		$scope.gradeId = gradeId;
    		//根据年级查询学科列表
    		url = jeucIp+"statistics/findSubjectByOid?gradeId="+gradeId
    		$http.get(url).success(function(response){
    			var subjctList = [];
    			if(response.ret==200){
    				angular.forEach(response.data,function(data,index){
    					var subjectObj = {};
    					subjectObj.subjectId = data.id;
    					subjectObj.subjectName = data.name;
    					subjctList.push = subjectObj;
    				})
    				$scope.subjectList = subjctList;
    			}
    		})
    	}
...
//			}
	...
	$scope.findResourceList = function(){
		...
		queryUrl += queryUrl+"&areaCodes="+$scope.areaCode;
